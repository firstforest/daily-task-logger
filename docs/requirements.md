...
- `--today` は日付フィルタの基準日とする。基準日より後のログ・コミット・journal 言及・ノート更新は、まだ存在しないものとして集計から除外する（`log_last` などは基準日以前の最新を採り、無ければ `null`）。経過日数が負になってはならない。先の日付を書いたログや翌日ぶんの journal も同様に扱う。
  - 巻き戻るのは日付由来のフィールド（`log_last` / `repo_last` / `journal_last` / `updated` と各日数、`logs`）だけ。`next_action` / `health` / `backlog` / `status` / `completed` はノートの現在の内容を出す（過去のノート内容は git から復元しない）。「その日時点のスナップショット」ではないので、ドキュメントでそう書かないこと。
