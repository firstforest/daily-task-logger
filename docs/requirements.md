...
- サブコマンド:
  ...
  - `list` — 日付別にタスクを表示。`--format json|yaml` で構造化出力、`--tag <tag>` でタグ絞り込み。構造化出力は該当0件でも空配列（`[]`）を返し、終了コードは 0 とする（呼び出し側が常にパースできるようにするため。人間向けのテキスト表示のときのみメッセージを出す）。
...
- `--today` は「その日時点の状態を再現する」基準日とする。基準日より後のログ・コミット・journal 言及・ノート更新は、まだ存在しないものとして集計から除外する（`log_last` などは基準日以前の最新を採り、無ければ `null`）。経過日数が負になってはならない。先の日付を書いたログや翌日ぶんの journal も同様に扱う。
