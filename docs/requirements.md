...
- サブコマンド:
  ...
  - `list` — 日付別にタスクを表示。`--format json|yaml` で構造化出力、`--tag <tag>` でタグ絞り込み。構造化出力の各タスクは原文（`text`）に加えて、判断メタデータを分離した `body` / `meta` を持つ（[2.3](#23-pj-ノートの日報形式) と同一のルール。利用側に同じ判定を再実装させないため）。
