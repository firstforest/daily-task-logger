...
Task                              -- Document 内のチェックボックス行
  ...
  ├ duration : Option<Duration>   -- どれくらいかかるか（時計時間）
  ├ weight   : Option<Weight>     -- どれくらい重いか（認知的な負荷）
...
**「判断メタデータ」という概念は無い。** `（45分・重・@PC）` は行末の括弧に属性を並べて書く**表層構文**（§4）であって、ドメインの概念ではない。括弧という*場所*で定義された概念は、セクションで定義された `next_action` / `backlog`（§2.2）と同じ category error である。括弧の中身は 4 種の独立した属性に分解される。

| 書き方 | 属性 | 何を表すか |
| --- | --- | --- |
| `45分` `2時間` | `duration` | 時計時間 |
| `軽` `重` | `weight` | 認知的な負荷。`duration` とは独立（15 分の重い判断も、3 時間の軽い作業もある） |
| `@PC` `@AI` `@家` | `contexts` | 着手の前提条件。ユーザーが自由に増やせるので閉じた集合ではない |
| `08-15` `締切08-15` | `deadline` | 期限 |
...
erDiagram
    ...
    TASK {
        ...
        Weight weight "軽 / 重（任意）"
        ContextSet contexts "@PC / @AI など"
...
| `Weight` | `軽` \| `重`。認知的な負荷 | （なし。同上） |
| `Context` | `@` に続く文字列。着手の前提条件。閉じた集合ではない | （なし。同上） |
...
| `Weight` / `Context` | 型に無い。`pj::split_decision_meta` が返す `meta` 文字列の一部 | G-12 |
...
- 目標: §2.1 のとおり `duration` / `weight` / `contexts` / `deadline` を Task の属性にする。行末の括弧は表層構文（§4）に退き、`meta` という概念は消える。
- 影響:
  ...
  | health | 「何が決まっていないか」を言えるようになる。ただし `Unclarified` の定義を変えるかは**未決** |
  | 未決 | 認識できない要素（`（45分・仮）` の `仮`）の扱い。本文に戻す / 落とす / 残差として保つ、のいずれか。現状は `meta` 文字列に含まれている |
  | 順序 | G-11 と同じ括弧を触る（所要時間・締切はどちらの課題にも現れる）ので、まとめて行うことになる |
