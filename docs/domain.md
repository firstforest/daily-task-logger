...
- **[design.md](design.md)** — *どう表しているか*（現状の型・構文・不変条件・写像）
...
**表層構文と走査手順はドメインではない。** 「Task をどう書くか」（行の文法）と「行をどう読むか」（走査の状態機械）は design.md §3・§4 の担当で、ここでは「Task が何であるか」だけを書く。行末の括弧やセクション見出しといった*場所*で概念を定義しない、というのが本ドキュメントの一貫した立場である。
