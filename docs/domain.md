...
**本ドキュメントは目標形である。** 現状の Rust の型はここと 1 対 1 に対応しておらず、差分は design.md の移行課題（G-*）に列挙してある。本ドキュメントの記述と実装が食い違う箇所には、必ず対応する G-* がある。実装より先に「何が何であるか」を固定するために分けて置いている。
...
```
attach(log) : Option<Task>        -- 直前の Task の配下に置かれた Log が、その Task に付く
```

**「配下」はインデント厳密大なりに加えて、間に浅い非空行を挟まないことを要求する。** 空行だけでは切れない。2 つ目の条件を落とすと、`## 今日の候補` に PJ を並べ、別の見出しの下に無関係な時刻付きログを書いた形（ジャーナルの普通の書き方である）で、そのログが候補に載せただけの Task に帰属してしまう。§5 の実働が言及と区別できなくなるので、**この条件は帰属の定義の一部であって走査の都合ではない。**

`attach` は実装でも 1 本である（`scan::Event::Log` の `task` フィールド。design.md §4）。走査の経路によって解釈が変わることはない。
...
`completed` は `status = Done` のときだけ意味を持つ。独立したフィールドにすると `active` かつ `completed: 2026-01-01` という無意味な組み合わせが表現できてしまう。状態に畳めば構文的に排除できる。front matter の表記（`project:` と `completed:` の 2 キー）は変えず、読み取ったあとの型だけを畳む（`ProjectState`）。
